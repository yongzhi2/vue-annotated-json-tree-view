<template>
  <div :class="getCSSClass(data)">
    <div v-if="isObject(data)" class="tree-view-item-leaf">
      ...
      <tree-view-item :key="getKeyId(data)" :root="getKeyId(data)" :max-depth="maxDepth" :current-depth="currentDepth+1" v-show="isOpen()" v-for="child in data.children" :data="child" :modifiable="modifiable" @change-data="onChangeData"></tree-view-item>
    </div>
